...
#import "PicBoxZoomingScrollView.h"
...
- (PicBoxZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index;
- (PicBoxZoomingScrollView *)pageDisplayingPhoto:(id<PicBoxImage>)photo;
- (PicBoxZoomingScrollView *)dequeueRecycledPage;
- (void)configurePage:(PicBoxZoomingScrollView *)page forIndex:(NSUInteger)index;
...
- (CGRect)frameForCaptionView:(PicBoxCaptionView *)captionView atIndex:(NSUInteger)index;
...
- (UIImage *)imageForPhoto:(id<PicBoxImage>)photo;
- (void)loadAdjacentPhotosIfNecessary:(id<PicBoxImage>)photo;
- (void)releaseAllUnderlyingPhotos;

@end

// PicBoxImageBrowser
@implementation PicBoxImageBrowser
...
- (id)init {
    if ((self = [super init])) {
        ...
        // Listen for PicBoxImage notifications
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(handlePicBoxImageLoadingDidEndNotification:)
                                                     name:PicBoxImage_LOADING_DID_END_NOTIFICATION
                                                   object:nil];
    }
...
- (id)initWithPhotoURLs:(NSArray *)photoURLsArray {
    if ((self = [self init])) {
        NSArray *photosArray = [PicBoxImage photosWithURLs:photoURLsArray];
        _photos = [[NSMutableArray alloc] initWithArray:photosArray];
    }
    return self;
}
...
- (void)panGestureRecognized:(id)sender {
    // Initial Setup
    PicBoxZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
    //PicBoxTapDetectingImageView *scrollView.photoImageView = scrollView.photoImageView;
...
- (void)performCloseAnimationWithScrollView:(PicBoxZoomingScrollView*)scrollView {
...
- (void)viewWillLayoutSubviews {
    ...
    for (PicBoxZoomingScrollView *page in _visiblePages) {
...
- (PicBoxCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    PicBoxCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <PicBoxImage> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[PicBoxCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (void)loadAdjacentPhotosIfNecessary:(id<PicBoxImage>)photo {
    PicBoxZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <PicBoxImage> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
                    PicBoxLog(@"Pre-loading image at index %i", pageIndex-1);
                }
            }
...
#pragma mark - PicBoxImage Loading Notification

- (void)handlePicBoxImageLoadingDidEndNotification:(NSNotification *)notification {
    id <PicBoxImage> photo = [notification object];
    PicBoxZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImage]) {
            // Successful load
...
- (void)tilePages {
    ...
    for (PicBoxZoomingScrollView *page in _visiblePages) {
        ...
        if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
            ...
            PicBoxLog(@"Removed page at index %i", PAGE_INDEX(page));
        }
    }
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            // Add new page
            PicBoxZoomingScrollView *page;
            page = [[PicBoxZoomingScrollView alloc] initWithPhotoBrowser:self];
            ...
            PicBoxLog(@"Added page at index %i", index);
            
            // Add caption
            PicBoxCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
...
- (PicBoxZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    PicBoxZoomingScrollView *thePage = nil;
    for (PicBoxZoomingScrollView *page in _visiblePages) {
...
- (void)configurePage:(PicBoxZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    __block __weak PicBoxImage *photo = (PicBoxImage*)page.photo;
    __weak PicBoxZoomingScrollView* weakPage = page;
...
- (PicBoxZoomingScrollView *)dequeueRecycledPage {
    PicBoxZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    id <PicBoxImage> currentPhoto = [self photoAtIndex:index];
...
- (void)actionButtonPressed:(id)sender {
    id <PicBoxImage> photo = [self photoAtIndex:_currentPageIndex];
